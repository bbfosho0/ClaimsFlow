...
export function auditText(file, content) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (/\b(?:TODO|FIXME|HACK)\b/.test(line)) {
      report('unfinished-marker', number, 'Remove unfinished implementation markers from deployed source.');
    }

    if (file.startsWith('frontend/src/app/') && /\bconsole\.(?:log|debug|info|warn|error)\s*\(/.test(line)) {
      report('console-debugging', number, 'Use application error handling instead of browser console output.');
    }

    if (file === 'frontend/src/app/app.routes.ts'
        && /path\s*:\s*['"](?:reports|settings)['"]/.test(line)) {
      report('placeholder-route', number, 'Reports and Settings are not deployable application routes.');
    }

    if (file === 'frontend/src/app/core/demo-role/demo-role.model.ts'
        && /\/app\/(?:reports|settings)\b/.test(line)) {
      report('placeholder-navigation', number, 'Reports and Settings are not role navigation destinations.');
    }

    if (file.startsWith('frontend/src/app/workspaces/')
        && basename(file) !== 'workspace-pages.component.ts'
        && /from\s+['"]\.\/(?:analytics|documents|team-operations)-page\.component['"]/.test(line)) {
      report('page-coupled-utility', number, 'Workspace pages must consume shared operational utilities, not another page component.');
    }

    if (file.endsWith('.html')
        && /\b(?:Upload Documents|OCR extraction|Storage used|Bulk Actions|Version history|Send SMS)\b/i.test(line)) {
...
async function main() {
  ...
  if (!findings.length) {
    console.log('Deployed source audit passed with zero findings.');
    return;
  }
