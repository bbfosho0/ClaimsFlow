...
export class NewClaimPageComponent {
  ...
  readonly today=localIsoDate(); readonly submitting=signal(false); readonly errors=signal<string[]>([]); readonly actionError=signal('');
  readonly form=this.fb.group({claimantName:['',[Validators.required,Validators.maxLength(160)]],claimantEmail:['',[Validators.required,Validators.email,Validators.maxLength(200)]],claimType:this.fb.control<ClaimType|''>('',Validators.required),incidentDate:['',[Validators.required,notFutureDate]],estimatedLoss:[0,[Validators.required,Validators.min(0)]],description:['',[Validators.required,Validators.minLength(20),Validators.maxLength(2000)]],incidentReportPresent:[false],photosPresent:[false],proofOfOwnershipPresent:[false],medicalDocumentationPresent:[false]});
